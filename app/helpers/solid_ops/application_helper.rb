...
module SolidOps
  module ApplicationHelper
    ...
    def status_pill(status)
      base = "inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-semibold ring-1 ring-inset"
      case status.to_s
      when "ready"     then "#{base} bg-yellow-50 text-yellow-800 ring-yellow-600/20"
      when "claimed"   then "#{base} bg-blue-50 text-blue-700 ring-blue-700/10"
      when "failed"    then "#{base} bg-red-50 text-red-700 ring-red-600/10"
      when "blocked"   then "#{base} bg-orange-50 text-orange-700 ring-orange-600/20"
      when "scheduled" then "#{base} bg-indigo-50 text-indigo-700 ring-indigo-700/10"
      when "finished"  then "#{base} bg-emerald-50 text-emerald-700 ring-emerald-600/20"
      when "paused"    then "#{base} bg-gray-50 text-gray-600 ring-gray-500/10"
      else "#{base} bg-gray-50 text-gray-600 ring-gray-500/10"
      end
    end
